...
class Panasonic::Projector::PPND < PlaceOS::Driver
  ...
  descriptive_name "Panasonic Projector PPND API (UCLA)"
  ...
  def on_update
    ...
    # Replace only our own status poll - a global schedule.clear would also
    # cancel the DeviceInfo interface's schedules
    @poll_task.try(&.cancel) rescue nil
    @poll_task = schedule.every(@poll_interval.seconds) do
      query_power_status
      query_input_status
      query_temperatures
  ...
  def device_info : Descriptor
    Descriptor.new(
      make: "Panasonic",
      model: self[:model]?.try(&.as_s).presence || "Projector",
