...
DriverSpecs.mock_driver "Shure::IntellimixRoom" do
  ...
  # the recurring live-state poll fires immediately; answer it so the serial
  # queue is free for the commands below
  should_send("< GET ALL >").responds "< REP DEVICE_AUDIO_MUTE OFF >"

  # device_info is served from the identity state cached above
  info = exec(:device_info).get.not_nil!
  info["make"].should eq("Shure")
  info["model"].should eq("INTELLIMIX_ROOM")
  info["firmware"].should eq("4.7.4")

  # Test presets (values publish as the raw strings the device reports)
  exec(:get_preset)
  should_send("< GET PRESET >").responds "< REP PRESET 3 >"
  status[:preset].should eq("3")

  # preset numbers are zero-padded to two digits on the wire
  exec(:set_preset, 5)
  should_send("< SET PRESET 05 >").responds "< REP PRESET 5 >"
