...
DriverSpecs.mock_driver "Epiphan::Pearl" do
  ...
  # Test device_info functionality
  retval = exec(:device_info)

  expect_http_request do |request, response|
    headers = request.headers
    if headers["Authorization"]? == "Basic #{Base64.strict_encode("admin:admin")}" && request.path == "/api/v2.0/system/firmware"
      response.status_code = 200
      response << %({
        "status": "ok",
        "result": {
  ...
  info["make"].should eq("Epiphan")
  info["model"].should eq("Pearl-2")
  info["firmware"].should eq("4.25.1")
end
