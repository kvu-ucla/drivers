...
# Version 2.0.1 — documentation: pearl_readme.md
...
class Epiphan::Pearl < PlaceOS::Driver
  ...
  descriptive_name "Epiphan Pearl Recording Device (UCLA)"
  ...
  @poll_task : PlaceOS::Driver::Proxy::Scheduler::TaskWrapper? = nil
  @poll_now_task : PlaceOS::Driver::Proxy::Scheduler::TaskWrapper? = nil

  def on_update
    @poll_every = setting?(Int32, :poll_every) || 30

    init_camera_map
    schedule_status_polling
  end

  def connected
    schedule_status_polling
    # firmware/identity is fetched by the interface-owned device_info call
    # (which also publishes the legacy firmware status via get_firmware)
  ...
  # replace only our own poll tasks - a global schedule.clear would also
  # cancel the DeviceInfo interface's schedules
  protected def schedule_status_polling
    @poll_task.try(&.cancel) rescue nil
    @poll_now_task.try(&.cancel) rescue nil
    @poll_task = schedule.every(@poll_every.seconds) { poll_status }
    @poll_now_task = schedule.in(2.seconds) { poll_status }
  end
  ...
  # the REST API only exposes firmware/product details; other fields degrade to nil
  def device_info : Descriptor
    firmware = begin
      get_firmware
    rescue error
      logger.warn(exception: error) { "firmware query failed, returning partial device info" }
      nil
    end

    Descriptor.new(
      make: "Epiphan",
      model: firmware.try(&.product_name).presence || "Pearl",
      firmware: firmware.try(&.version),
