...
class Sony::Camera::CGI < PlaceOS::Driver
  ...
  generic_name :Camera
  descriptive_name "Sony Camera HTTP CGI Protocol (UCLA)"
  ...
  def on_load
    ...
    # identity acquisition (inq=system) is owned by the DeviceInfo interface
    # schedule - only live state is polled here
    schedule.every(60.seconds) { query_status }
    schedule.in(5.seconds) { query_status }
    on_update
  end
  ...
  def query_status(priority : Int32 = 0)
    ...
    status_query = query("/command/inquiry.cgi?inq=ptzf", priority: priority) do |response|
    ...
    autoframing?
    power?
    # return the ptzf query so exec/.get resolves with the PTZ state, not the
    # power? task appended above (exec awaits a returned Task)
    status_query
  end
  ...
  # convert a CGI inquiry response body into more consumable state
  private def parse_inquiry(data : String) : Hash(String, String)
  ...
  # fields degrade to nil if the system inquiry fails
  def device_info : Descriptor
    state = begin
      response = get_with_digest_auth("/command/inquiry.cgi?inq=system")
      raise "unexpected response #{response.status_code}" unless response.success?
      parse_inquiry(response.body.not_nil!)
    rescue error
      logger.warn(exception: error) { "system inquiry failed, returning partial device info" }
      {} of String => String
    end

    # keep the legacy identity status keys published from this same fetch
    # (previously published by the scheduled info? query)
    state.each do |key, value|
      self[key.underscore] = value if {"ModelName", "Serial", "SoftVersion", "ModelForm", "CGIVersion"}.includes?(key)
    end

    Descriptor.new(
      make: "Sony",
      model: state["ModelName"]?.presence || "Camera",
      serial: state["Serial"]?,
      firmware: state["SoftVersion"]?,
